...
B.) Model Lookup
- This problem uses pandas filtering to find specific car models. First, it selects the complete row of the Toyota Corolla by checking which row model is equal to "Toyota Corolla". Then, it uses .iloc[] to find the Pontiac Firebird and display only its Model, mpg, hp, and wt columns.

C.) Multi-Model Subsetting
- For this last problem, the code demonstrates multi-model subsetting. It uses .isin() to filter the cars DataFrame and select only the specified rows. It then selects only the columns Model, mpg, cyl, hp, and gear. Finally, it displays the selected card, which uses .shape to show the number of rows and columns in the filtered DataFrame.
